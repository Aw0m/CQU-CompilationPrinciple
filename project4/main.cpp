...
//    map<string, Symbol> globalSymbolTable;
    ...
    vector<string> _text;
    vector<string> _main;
    vector<string> _data = {"newLine: .asciiz \"\\n\""};
    int dataCounter = 0;
    map<Variable*, string> varToReg;
    // false 表示该register还没有被使用
    vector<pair<string, bool>> registers = {
            {"$s0", false}, {"$s1", false}, {"$s2", false}, {"$s3", false},
            {"$s4", false},{"$s5", false}, {"$s6", false}, {"$s7", false},
    };

    //TODO getReg, mipsInitReg 负责常量、变量的初始化、定义
    string getReg() {
    ...
    void mipsInitReg(Variable* var, const string& val, bool init) {
        string res = getReg();
        varToReg[var] = res;
        if (init) {
            string msg = "li " + res + " " + val;
            _main.emplace_back(msg);
        }
    }

    void mipsNewLine() {
        _main.emplace_back("\nli, $v0, 4");
        _main.emplace_back("la $a0, newLine");
        _main.emplace_back("syscall\n");
    }

    //TODO PRINTF mips负责输出的部分
    string outString(const string& str) {
        string printMsg = "printMsg" + to_string(dataCounter++);
        string res = printMsg + ": .asciiz " + "\"" + str + "\"";
        _data.push_back(res);
        return printMsg;
    };
    void mipsOutString(const string& str) {
        _main.emplace_back("\nli, $v0, 4");
        _main.emplace_back("la $a0, " + this->outString(str));
        _main.emplace_back("syscall\n");
    }
    void mipsOutVal(const string& reg) {
        _main.emplace_back("\nli, $v0, 1");
        _main.emplace_back("move $a0, " + reg);
        _main.emplace_back("syscall\n");
    }

    //TODO SCANF mips负责读的部分
    void mipsScanf(Variable* var) {
        string msg = "move " + varToReg[var] + ", $v0" ;
        _main.emplace_back("\nli, $v0, 5");
        _main.emplace_back("syscall");
        _main.emplace_back(msg);
        _main.emplace_back("\n");
    ...
    void mipsAssign(Variable* var, const string& reg) {
        string msg = "move " + varToReg[var] + ", " + reg ;
        _main.emplace_back(msg);
    }

    void mipsAdd(const string& reg1, const string& reg2) {
        string msg = "add " + reg1 + ", " + reg1 + ", " + reg2;
        _main.emplace_back(msg);
    }

    void mipsMove(const string& reg1, const string& reg2) {
        string msg = "move " + reg1 + ", " + reg2;
        _main.emplace_back(msg);
    }

    void mipsMulti(const string& reg1, const string& reg2) {
        string msg1 = "mult " + reg1 + ", " + reg2;
        string msg2 = "mflo " + reg1;
        _main.emplace_back(msg1);
        _main.emplace_back(msg2);
    }

    void mipsDiv(const string& reg1, const string& reg2) {
        string msg1 = "div "  + reg1 + ", " + reg2;
        string msg2 = "mflo " + reg1;
        _main.emplace_back(msg1);
        _main.emplace_back(msg2);
    }
    ...
    void outMips() {
        ...
        mipsOut << endl << ".text:" << endl;
        mipsOut << endl << "main:" << endl;
        for (const auto& x : _main) {
            mipsOut << x << endl;
        }
    }
    ...
    void constDefine(ofstream& out) {
        ...
        auto var = identifierVarError(varName, true, varTempType);
        string val;
        // 标识符
        outputLexRes(out);
        // =
        ...
        //TODO 初始化
        mipsInitReg(var, val, true);
        while (lexRes[currPos].first == "COMMA") {
            outputLexRes(out);
            varName = lower(lexRes[currPos].second);
            var = identifierVarError(varName, true, varTempType);
            outputLexRes(out);
            outputLexRes(out);
            if (varTempType == "INTTK" ) {
                if (lexRes[currPos].first == "CHARCON") {
    ...
    void varDecribe(ofstream& out) {
        while (varIDENFR(lexRes[currPos].first) && (lexRes[currPos + 2].first) != "LPARENT") {
            ...
            do {
                ...
                // 修改的常量定义的 名字重定义 判断
                string varName = lower(lexRes[currPos].second);
                auto var = identifierVarError(varName, false, typeTemp);
                mipsInitReg(var, "", false);
                // 标识符
                outputLexRes(out);
...
//        currPos--;
    ...
    Variable* identifierVarError(const string& varName, bool isConst, const string& type) {
        if (this->scope == 0) {
    ...
    void statement(ofstream& out) {
        ...
        if (lexRes[currPos].first == "SEMICN") {
            ...
            if (lexRes[currPos].first == "STRCON") {
                // 字符串
                //TODO PRINTF 字符串
                mipsOutString(lexRes[currPos].second);
                outputLexRes(out);
                out << "<字符串>" << endl;
...
//            auto func = funcTable[this->currFunc];
    ...
    string term(ofstream& out) {
        string termResReg = "$t1";
        string resType;
        resType = factor(out);
        mipsMove(termResReg, "$t2");
        bool intType = false;
        while (lexRes[currPos].first == "MULT" || lexRes[currPos].first == "DIV") {
            bool flag = (lexRes[currPos].first == "MULT");
            intType = true;
            outputLexRes(out);
            factor(out);
            if (flag) {
                mipsMulti(termResReg, "$t2");
            } else {
                mipsDiv(termResReg, "$t2");
            }
        }
        out << "<项>" << endl;
        if (intType) {
    ...
    string factor(ofstream& out) {
        ...
        if (funcResType.find(lexRes[currPos].second) != funcResType.end()) {
            ...
            string val = lexRes[currPos].second;
            string msg = "li $t2, " + val;
            _main.push_back(msg);
            ...
            string sign = lexRes[currPos].first == "PLUS" ? "+" : "-";
            string val = sign + lexRes[currPos+1].second;
            string msg = "li $t2, " + val;
            _main.push_back(msg);
            ...
            mipsMove(factorResReg, varToReg[var]);
    ...
    string intNum(ofstream& out) {
        string val;
        if (lexRes[currPos].first == "PLUS" || lexRes[currPos].first == "MINU") {
            // + | -
            val = lexRes[currPos].first == "PLUS" ? "+" : "-";
            outputLexRes(out);
        }
        val += lexRes[currPos].second;
        unsignedInteger(out);
        out << "<整数>" << endl;
        return val;
    }
